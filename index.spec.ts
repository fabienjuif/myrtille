...
/* eslint-env jest */
import createStore from './packages/mutate/index'
...
it('should set state', () => {
  const store = createStore<any, any>({ before: true })
...
it('should listen to dispatch', () => {
  ...
  expect(reaction.mock.calls[1]).toEqual([
    store,
    { type: 'ADD_TODO', payload: { id: 2, label: 'new' } },
  ])
})
...
it('should listen to function', () => {
  ...
  store.dispatch({ type: 'unknown', payload: 2 })
...
it('should be possible to dispatch an action into a listener', () => {
  ...
  store.addListener('FETCH_TODOS', (store) => {
    store.dispatch({
      type: 'SET_TODOS',
      payload: [{ id: 303, label: 'imagine it is fetched' }],
    })
  })
  ...
  expect(reaction).toHaveBeenCalledTimes(1)
  expect(reaction.mock.calls[0]).toEqual([
    store,
    {
      type: 'SET_TODOS',
      payload: [{ id: 303, label: 'imagine it is fetched' }],
...
it('should be possible to mutate store into a subscriber', () => {
  ...
  store.dispatch({
    type: 'ADD_TODO',
    payload: { id: 1, label: 'new', user: 'Delphine' },
  })

  expect(store.getState()).toEqual({
    name: 'Delphine',
    todos: [{ id: 1, label: 'new', user: 'Delphine' }],
  })
})
...
it('should be possible to dispatch an action into a subscriber', () => {
  ...
  store.subscribe('name', (store) => {
    store.dispatch({
      type: 'ADD_TODO',
      payload: { id: 1, label: 'Introduce yourself to #welcome' },
    })
  })
  store.addListener('ADD_TODO', reaction)
